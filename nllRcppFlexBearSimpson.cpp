...
// -----------------------------------------------------------------------------
// [[Rcpp::export]]
double uncondNllRcppVarSL(List dat, List par) {
  // ===========================================================================
  ...
  for (int sl_index = 0; sl_index < n_sl; sl_index++) {
    ...
    lambda_vector[sl_index] = sum(lambda_vector_sl) * S_probs[sl_index];
  }
  
  // std::cout << "The value for lambda is: " << lambda << std::endl;
  
  // Do the Simpson's rule stuff
  NumericVector simpson_rule_vector(n_sl);
  
  for (int index = 0; index <= n_sl; index++) { // index less OR EQUAL to n_sl <<- very important!
    if (index == 0 | index == n_sl) {
      simpson_rule_vector[index] = lambda_vector[index];
    } else if (index % 2 == 0) {
      simpson_rule_vector[index] = 2.0 * lambda_vector[index]; 
    } else {
      simpson_rule_vector[index] = 4.0 * lambda_vector[index];
    }
  }
  
  double lambda = A_s / 3 * sum(simpson_rule_vector);
...
// [[Rcpp::export]]
double singleNllRcppVarSL(List dat, List par) {
  // ===========================================================================
  // 1. EXTRACT DATA OBJECTS
  // ---------------------------------------------------------------------------
  ...
  for (int index = 0; index < n_sl; index++) {
    
    if (index == 0 | index == n_sl - 1) {
      simpson_rule_vector[index] = log_probs_i[index];
      std::cout << "simpson_rule_vector[" << index << "] is: " <<  log_probs_i[index] << std::endl;
    } else if (index % 2 == 0) {
      simpson_rule_vector[index] = log(2.0) + log_probs_i[index];
      std::cout << "simpson_rule_vector[" << index << "] is: log(2.0) + " <<  log_probs_i[index]<< std::endl;
    } else {
      simpson_rule_vector[index] = log(4.0) + log_probs_i[index];
      std::cout << "simpson_rule_vector[" << index << "] is: log(4.0) + " <<  log_probs_i[index]<< std::endl;
      
    }
    // std::cout << "simpson_rule_vector[" << index << "] is: " <<  simpson_rule_vector[index] << std::endl;
  }
  
  // Sum the values in log_probs_i for every source level, in a similar way to logsumexp()
  double log_sum_i = simpson_rule_vector[0];
  for (int ii = 1; ii < n_sl; ii++) {
    log_sum_i = R::logspace_add(log_sum_i, simpson_rule_vector[ii]);
  }
  // Make it negative
  double nll_cond = - log_A_s - log(3) + log_sum_i;  // log(A_s / 3 * whole summation)
