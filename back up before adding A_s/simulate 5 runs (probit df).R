...
system.time({
  for (bw_list_index in 1:1) {
    ...
    if (det_function == "janoschek") {
      ...
      # Detection function parameters
      par_det <- c(U = 0.8, # should be in (0, 1]
                   B = 8, # should be in (0, Inf)
                   Q = 2) # should be in (0, Inf)
    } else {
      par_det <- c(g0 = 0.7,
                   sigma = 15000)
    }
    ...
    if(det_function == "janoschek") {
      ...
      par_det_start <- c(U = log(0.5 / (1 - 0.5)), # should be in (0, 1]
                         B = log(15), # should be in (0, Inf)
                         Q = log(1)) # should be in (0, Inf)
    } else {
    ...
    par_sl_start <- c(mu_s = log(110), # identity
                      sd_s = log(10)) # log
    ...
    save(bw_list, file = "temp_sim_5_runs_BFGS_factr1e10_index=1.RData")
  }
})
