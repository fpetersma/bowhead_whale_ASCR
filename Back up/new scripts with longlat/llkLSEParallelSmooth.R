...
.llkLSEParallelSmooth <- function(par, dat) {
  ...
  if (USE_RL) {
    ...
    if (!CONSTANT_DENSITY) {
      ...
      rate <- A_s * A_x * mean(unlist(rates_noise)) # '* A_x' is old, as A_x is now variable
    ...
    cond_llk_call <- parLapply(cl, as.list(1:n_call), function(i) { 
      ...
      if (USE_RL) {
        ...
        part_source_level <- matrix(log(part_source_level), nrow = n_grid, 
                                    ncol = n_sl, byrow = FALSE) #THIS MIGHT BE FINE
      ...
      part_4 <- matrixStats::logSumExp(temp) + log(A_x) + log(A_s) # OLD + log(A_x) + log(A_s)
      # Replace -Inf with -6000, since exp(-6000) = 0
