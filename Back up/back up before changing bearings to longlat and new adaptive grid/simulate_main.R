...
# source("Scripts/Bowhead Whales/llkLSEv2.R")
...
density_grid <- read.csv("Data/JABES paper/grid_adaptive_levels=1_maxD2C=100k_maxD2A=100k_area=21079.44_n=375_UTM_zone=7.csv")
cov_density <- density_grid
colnames(cov_density)[c(2,1)] <- c("y", "x")
cov_density$depth <- abs(cov_density$depth)
...
cov_density[["dist_to_coast2"]] <- cov_density$distance_to_coast ^ 2
...
index <- 1:nrow(dat_sim$det_hist) #index <- sample(1:nrow(dat_sim$det_hist), sample_size)
...
mask <- read.csv("Data/JABES paper/grid_adaptive_levels=1_maxD2C=100k_maxD2A=100k_area=21079.44_n=375_UTM_zone=7.csv")
