...
bwASCR <- function(dat, par, method = "L-BFGS-B", maxit = 100, TRACE = TRUE,
  ...
  fit_duration <- ({
    ...
    result <- optim(par = par, fn = fn, method = method, hessian = TRUE,
                    control = list(maxit = maxit, fnscale = -1, trace = TRACE, 
                                   REPORT = 1, factr = 1e10),
                    lower = c(U = -5, B = -5, Q = log(0.01), kappa = log(1), beta_r = log(5), sd_r = log(0.01), mu_s = log(0.01),  sd_s = log(0.01), rep(-Inf, 100)), # this gives 0 on log and logit link
                    upper = c(U = 5, B = 5, Q = log(10), kappa = log(100), beta_r = log(30), sd_r = log(50),   mu_s  =log(300) , sd_s = log(50), rep(Inf, 100)), # this gives 2.7e10 on log and 1 on logit
  ...
  output <- list(N = N, estimates = estimates, real = real, 
                 ...
                 ESA = esa, optim_result = result, fit_duration = fit_durations, 
                 data = dat)
