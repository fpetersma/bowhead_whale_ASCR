...
bwASCR <- function(dat, par, method = "L-BFGS", maxit = 100, TRACE = TRUE,
  ...
  result <- optim(par = par, fn = fn, method = method, hessian = FALSE,
                  control = list(maxit = maxit, fnscale = -1, trace = TRACE, 
                                 REPORT = 1, factr = 1e11),
                  # lower = c(U = -5, B = -5, Q = log(1), beta_r = log(10), sd_r = log(0.1), mu_s = log(70),  sd_s = log(1)), # this gives 0 on log and logit link
                  # upper = c(U = 5, B = 5, Q = log(10), beta_r = log(30), sd_r = log(10),   mu_s  =log(130) , sd_s = log(20) ), # this gives 2.7e10 on log and 1 on logit
