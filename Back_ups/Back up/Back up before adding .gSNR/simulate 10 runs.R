bw_list <- list()
system.time({
for (bw_list_index in 6:10) {
  cat("Working on fit nr: ", bw_list_index, "\n")
  ################################################################################
  ...
  # par_dens <- c("(Intercept)" = -2.2, "dist_to_coast" = 2.5, "dist_to_coast2" = -1.5)
  ...
  if(det_function == "jano") {
    par_det_start <- c(U = log(0.8 / (1 - 0.8)), # should be in (0, 1]
                       B = log(2), # should be in (0, Inf)
                       Q = log(4 - 1)) # should be in (1, Inf)
  } else {
  ...
  # par_dens_start <- c("(Intercept)" = -2.2, "dist_to_coast" = 2.5, "dist_to_coast2" = -1.5)
  ...
  bw_list[[bw_list_index]] <- bw
  
  # Saving on object in RData format
  save(bw_list, file = "bw simulation.RData")
}
})
