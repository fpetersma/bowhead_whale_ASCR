...
bwASCR <- function(dat, par, method = "L-BFGS-B", maxit = 1000, TRACE = TRUE,
                   LSE = TRUE) {
  ...
  if (dat$det_function == "janoschek" | dat$det_function == "logit" | dat$det_function == "probit") {
    ...
    stop(paste0("Detection function specification is ", dat$det_function,
                ", but should be 'janoscheck', 'logit' or 'probit' or 'half-normal'"))
  }
  ...
  result <- optim(par = par, fn = fn, method = method, hessian = TRUE,
                  control = list(maxit = maxit, fnscale = -1, trace = TRACE, 
                                 REPORT = 1),# factr = 1e10),
                  lower = c(U = -5, B = -5, Q = log(0.01), kappa = log(1), beta_r = log(5), sd_r = log(0.01), mu_s = log(0.01),  sd_s = log(0.01)), # this gives 0 on log and logit link
                  upper = c(U = 5, B = 5, Q = log(10), kappa = log(100), beta_r = log(30), sd_r = log(50),   mu_s  =log(300) , sd_s = log(50) ), # this gives 2.7e10 on log and 1 on logit
