...
detections <- read.csv("Data/JABES paper/detections_31-08-2020_successful-loc.csv")
bearings <- read.csv("Data/JABES paper/bearings_31-08-2020_successful-loc.csv")
# weights <- read.csv("Data/Revised data with different cut-offs [22-11-19]/[PT] weights_history_S5Y10_0.02.csv")
received_levels <- read.csv("Data/JABES paper/received_levels_31-08-2020_successful-loc.csv")
noise_call <- read.csv("Data/JABES paper/noise_31-08-2020_successful-loc.csv")
...
if(det_function == "janoschek") {
  ...
  par_det_start <- c(U = log(0.6 / (1 - 0.6)), # should be in (0, 1]
                     B = log(1.5), # should be in (0, Inf)
...
# par_dens_start <- c("(Intercept)" = -3, "distance_to_coast" = 1, "distance_to_coast2" = -1.5)
