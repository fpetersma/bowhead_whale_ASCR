...
seeds <- as.list(seq(from = 170022736, by = 121, length = 20))

################################################################################
####                      SIMULATE THE DATA                                 ####
################################################################################

sim_data <- lapply(seeds, function(seed) {
  ...
  # source("Scripts/Bowhead Whales/llkLSEParallelSmoothNoSLSingleCore.R")
  ...
  density_grid <- read.csv("Data/Masks/[PT] mask_buffer=75km_mesh=coarse_A=26862864m2_deltaX=5158_deltaY=5208_grid=406.csv")
  ...
  par_dens <- c("(Intercept)" = -2.9, "dist_to_coast" = 2, "dist_to_coast2" = -2)
...
####                         SEQUENTIAL FITTING                             ####
################################################################################
output_seq <- lapply(sim_data[1], function(dat_sim) {
...
output_par2 <- parLapply(cl, sim_data[1], function(dat_sim) {
  ################# Fit ASCR model to the simulated data #######################
  ## Load required libraries
  library("readr")
  library("matrixStats")
