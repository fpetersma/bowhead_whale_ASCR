  ...
  if (USE_RL) {
    ...
    if (!CONSTANT_DENSITY) {
      ...
      rates_noise <- parLapply(cl, noise_random_list, function(c) {
        ...
        probs_sl <- 1 # <- truncnorm::dtruncnorm(x = source_levels, a = lower_s,
      ...
      rate <- mean(unlist(rates_noise)) # * A_s
    ...
    cond_llk_call <- parLapply(cl, as.list(1:n_call), function(i) { 
      ...
      if (USE_RL) {
        ...
        part_received_levels <- t(part_received_levels)
        
        part_source_level <- matrix(1, nrow = n_grid, ncol = n_sl, byrow = TRUE)
      ...
      if (USE_RL) {
        ...
        temp <- log(p.) + part_density + part_source_level + log(A_x$area)
      ...
      if (USE_RL) {
        ...
        total <- matrixStats::logSumExp(pred) # + log(A_x$area)
