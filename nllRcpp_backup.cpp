...
double singleNllRcpp(List dat, List par) {
  ...
  // 1. EXTRACT DATA OBJECTS
  // ---------------------------------------------------------------------------
  const NumericMatrix W = dat["W"];            // detection histories
  const NumericMatrix R = dat["R"];            // received levels
  const NumericMatrix Y_rec = dat["Y_rec"];    // recorded bearings
  ...
  const NumericVector D = dat["D"];            // density for every grid point
  const double bessel_low = dat["bessel_low"]; // the BesselI() values
  const double bessel_high = dat["bessel_high"];
  // ===========================================================================
